...
if (!function_exists('multi_category')) {
    function multi_category($data, $parent_id = 0, $level = 0)
    {
        $result = [];
        foreach ($data as $key => $item) {
            if ($item['c_parent_id'] == $parent_id) {
                ...
                unset($data[$item['id']]);
                //--duyệt mảng con
                $child = multi_category($data, $item['id'], $level + 1);
...
if (!function_exists('pare_url_file')) {
    ...
    {
        if (!$image) {
            return 'backend/build/images/default.jpg';
        }
...
if (!function_exists('currency_format')) {
    function currency_format($number, $flug = 'đ')
    {
        return number_format($number) . ' ' . $flug;
    }
}

if (!function_exists('get_price_sale')) {
    function get_price_sale($number, $percent)
    {
        $priceSale = $number * 100 / (100 - $percent);
        return $priceSale;
    }
}
